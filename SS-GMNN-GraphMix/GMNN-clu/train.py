...
parser.add_argument('--partition_num', type=int, default=14, help='partition number')
parser.add_argument('--task_ratio', type=float, default=0.8, help='task ratio')
...
cluster_labels_file = './cluster_labels/pubmed.npy'
partition_labels = torch.tensor(np.load(cluster_labels_file), dtype=torch.int64).cuda()
...
idxss = range(opt['num_node'])
idx_unlabel = [x for x in idxss if x not in idx_train]
...
def pre_train(epoches):
    ...
    for epoch in range(epoches):
        loss = trainer_q.update_soft(inputs_q, target_q, idx_train, idx_unlabel)
...
def train_p(epoches):
    ...
    results = []
    for epoch in range(epoches):
        loss = trainer_p.update_soft(inputs_p, target_p, idx_all, idx_unlabel)
...
def train_q(epoches):
    ...
    results = []
    for epoch in range(epoches):
        loss = trainer_q.update_soft(inputs_q, target_q, idx_all, idx_unlabel)
        _, preds, accuracy_dev = trainer_q.evaluate(inputs_q, target, idx_dev)
        _, preds, accuracy_test = trainer_q.evaluate(inputs_q, target, idx_test)
        results += [(accuracy_dev, accuracy_test)]
    return results
