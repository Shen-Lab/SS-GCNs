...
f_mask, _, _ = inputs.svd()
partition_labels = f_mask[:, :opt['partition_num']]
d = partition_labels.sum(dim=1)
d[abs(d)<1e-2] = 1e-2
partition_labels = partition_labels / d.view(opt['num_node'], 1)


gnn = GNN(opt, adj)
trainer = Trainer(opt, gnn, partition_labels, opt['partition_num'])
...
for param in gnn_ema.parameters():
            param.detach_()
trainer_ema = Trainer(opt, gnn_ema, partition_labels, opt['partition_num'], ema = False)
...
def train(epoches):
    ...
    for epoch in range(epoches):
        ...
        if rand_index == 0:
            ...
            loss , loss_usup, loss0 = trainer.update_soft_aux(inputs_q, target_q, target, idx_train, idx_unlabeled_subset, adj, adj_ss,  opt, mixup_layer =[1])
            mixup_consistency = get_current_consistency_weight(opt['mixup_consistency'], epoch)
            total_loss = loss + mixup_consistency*loss_usup

            total_loss = total_loss + loss0 * opt['task_ratio']
            ...
            trainer.model.train()
            trainer.optimizer.zero_grad()
            loss, loss0 = trainer.update_soft(inputs_q, target_q, idx_train, adj_ss)
            
            total_loss = loss + loss0 * opt['task_ratio']
