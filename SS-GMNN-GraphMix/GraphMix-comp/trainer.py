...
class Trainer(object):
    def __init__(self, opt, model, partition_labels, pn, ema= True):
    ...
    def update_soft(self, inputs, target, idx, adj_ss):
        ...
        logits0 = self.model.forward_partition(inputs, adj_ss.cuda())
        logits0 = self.ss_classifier(logits0)
        loss0 = self.loss_func(logits0, self.partition_labels)
        
        return loss, loss0
    
    def update_soft_aux(self, inputs, target,target_discrete, idx, idx_unlabeled, adj, adj_ss, opt, mixup_layer):
